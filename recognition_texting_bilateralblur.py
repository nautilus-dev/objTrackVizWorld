...
def main(args=sys.argv[1:]):
    video = cv2.VideoCapture("videos/shortcut_gorilla.avi")
    ...
    cv2.waitKey(0)
    # Gaussian Blur -> worse result
    # blur = cv2.GaussianBlur(frame, (15,15), 50)
    # cv2.imshow("Blur", blur)
    # cv2.waitKey(0)
    # gray_frame = cv2.cvtColor(blur, cv2.COLOR_BGR2GRAY)

    blur = cv2.bilateralFilter(frame,100,75,700)
    cv2.imshow("Blur", blur)
    cv2.waitKey(0)
    gray_frame = cv2.cvtColor(blur, cv2.COLOR_BGR2GRAY)
    ...
    for c in contours:
        ...
        # just bigger boxes - later compare with the size of the last bounding box 
        if abs(x + w) > abs(x + 60 ) and abs(y + h) > abs(y + 60) and abs(x + w) < abs(x + 500) and abs(y + h) < abs(y + 500) :
